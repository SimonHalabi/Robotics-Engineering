...
int main(int argc, char *argv[]) {
   wb_robot_init();
   //WbNodeRef node = wb_supervisor_node_get_from_def("e-puck2");
   printf("hi");
     if (strcmp(wb_robot_get_model(), wb_robot_get_name()) == 0) {
    ...
    //printf("hi9");
   // }
  WbDeviceTag light_sensor[8], left_motor, right_motor, tx1, rx1;
...
printf("hi7");
 ...
 printf("hi8");
  ...
  while (wb_robot_step(time_step) != -1) {
   ...
   printf("hi4"); 
   if(pso_array[3][0] == 0 && pso_array[3][1] == 0)
   { 
   bool *is_sensors_active = get_sensors_condition();
    ...
    tx1 = wb_robot_get_device("emitter");
          //wb_transmitter_enable(tx1, time_step);   
  wb_emitter_set_channel(tx1, WB_CHANNEL_BROADCAST);
